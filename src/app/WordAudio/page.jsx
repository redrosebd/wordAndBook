...
function Page() {
  ...
  const handleInputFieldClick = () => {
    const englishText = new SpeechSynthesisUtterance(inputValue);
    englishText.voice = speechSynthesis.getVoices()[0];
    speechSynthesis.speak(englishText);
  };

  /* -------For English Words form Table Row ------------- */
  const handleWorldClick = (event) => {
    const word = event.target.textContent;
    const englishWord = new SpeechSynthesisUtterance(word);
    englishWord.voice = speechSynthesis.getVoices()[0];
    speechSynthesis.speak(englishWord);
  };
